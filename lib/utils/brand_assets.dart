...
class BrandAssets {
  ...
  static final List<MapEntry<RegExp, String>>
  _mapping = <MapEntry<RegExp, String>>[
    ...
    // [kelivo-hosted] kelivo-arch.md §8 — hosted provider display name is
    // "Ethan's AI" (settings_provider.dart), reuse the app icon as its logo.
    MapEntry(RegExp(r'ethan'), 'ethans_ai.png'),
  ...
  // [kelivo-hosted] kelivo-arch.md §8 — the "Ethan's AI" hosted-provider logo
  // is already a full-bleed circular mark (no built-in padding), unlike most
  // brand logos here which are square-canvas marks meant to be inset within
  // their circular badge. Full-bleed assets should fill their container
  // instead of being scaled down to ~62-70%.
  static bool assetIsFullBleed(String asset) => asset.endsWith('ethans_ai.png');
